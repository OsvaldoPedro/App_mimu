...
export default function Navbar() {
  ...
  const userAvatar = user ? (
    userPhoto ? (
      <OptimizedImage src={userPhoto} alt="Avatar" className="w-14 h-14 md:w-16 md:h-16 rounded-full border-2 border-mimu-gold" objectFit="cover" />
    ) : null
  ) : null;
        ...
        <div className="flex items-center justify-between h-24 sm:h-28">
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <Link to="/" className="flex items-center group">
              <img src="/mimu-logo.png" alt="Mimu Logo" className="h-[76px] sm:h-[90px] w-auto object-contain transition-transform group-hover:scale-105" />
            </Link>
          </div>
          ...
          <div className="hidden lg:flex items-center gap-6">
            ...
            <button
              onClick={toggleTheme}
              className={`w-14 h-14 flex items-center justify-center rounded-xl transition-all duration-300 border ${theme === 'dark' ? 'bg-[#1E1E1E] border-[#2A2A2A]' : 'bg-white dark:bg-[#1E1E1E]/10 border-white/20 hover:bg-white dark:bg-[#1E1E1E]/20'}`}
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 px-5 py-3.5 rounded-2xl text-mimu-white-text/90 hover:bg-mimu-wine-light dark:hover:bg-[#1A1A1A] transition-colors"
              >
                ...
                <span className="text-lg font-bold">{t('nav.language')}</span>
            ...
            {user ? (
              <>
                ...
                <div className="flex items-center gap-5 ml-4 pl-8 border-l border-[#5C1A1A]">
                  <Link 
                    to={`${dashboardPath}?tab=notificacoes`}
                    className="relative p-3 text-mimu-white-text/90 hover:text-mimu-gold transition-colors focus:outline-none shrink-0"
                    title="Notificações"
                  >
                    ...
                    {unreadCount > 0 && (
                      <span className="absolute top-1 right-1 flex h-6 w-6 items-center justify-center rounded-full bg-red-500 text-sm font-bold text-white ring-2 ring-mimu-wine dark:ring-[#121212] animate-pulse">
                        {unreadCount}
                      </span>
                    )}
                  </Link>
          ...
          <div className="lg:hidden flex items-center gap-4 sm:gap-6">
            {user && (
              <>
                ...
                >
                  ...
                  {unreadCount > 0 && (
                    <span className="absolute top-1 right-1 flex h-5 w-5 items-center justify-center rounded-full bg-mimu-gold text-[10px] font-black text-black ring-2 ring-mimu-wine dark:ring-[#121212]">
                      {unreadCount}
                    </span>
                  )}
                </Link>
                <Link to={dashboardPath} className="transition-transform hover:scale-105 shrink-0 p-1">
                  {userAvatar}
                </Link>
              </>
            )}
            ...
            <button
              onClick={toggleTheme}
              className="w-14 h-14 flex items-center justify-center rounded-full text-mimu-white-text hover:text-mimu-gold hover:bg-white/10 dark:hover:bg-white/5 transition-all duration-300"
              title={theme === 'dark' ? 'Light Mode' : 'Dark Mode'}
            >
              {theme === 'dark' ? (
                <svg className="w-8 h-8 text-mimu-gold fill-current" viewBox="0 0 24 24">
