...
export default function ClientDashboard() {
  ...
  const handleVerifyDashboardPayment = async (payment) => {
    const toastId = toast.loading(t('dashboard.verifyingPayment'))
    try {
      const res = await getAppyPaymentStatus(payment.id, payment.transaction_id)
      if (res.success && res.data) {
        if (res.data.status === 'paid') {
          toast.success(t('dashboard.paymentConfirmed'), { id: toastId })
          fetchPayments()
          reload()
        } else if (res.data.status === 'pending') {
          toast.error(t('dashboard.paymentStillPending'), { id: toastId })
        } else {
          toast.error(`O pagamento foi atualizado para: ${res.data.status}`, { id: toastId })
          fetchPayments()
        }
      } else {
        toast.error(res.error || t('dashboard.paymentError'), { id: toastId })
      }
    } catch (e) {
      console.error(e)
      toast.error(t('dashboard.paymentError'), { id: toastId })
    }
  }

  const handleCancelDashboardPayment = async (payment) => {
    if (!window.confirm(t('dashboard.confirmCancelPayment'))) return
    const toastId = toast.loading(t('dashboard.cancellingPayment'))
    try {
      const res = await cancelAppyPayment(payment.id, payment.transaction_id)
      if (res.success) {
        toast.success(t('dashboard.paymentCancelled'), { id: toastId })
        fetchPayments()
        reload()
      } else {
        toast.error(res.error || t('dashboard.paymentError'), { id: toastId })
  ...
  const submitReview = async (e) => {
    ...
    if (res.success) {
      toast.success(t('dashboard.reviewSuccess'))
  ...
  const handleCancel = (orderId, status) => {
    ...
    const ok = window.confirm(t('dashboard.confirmCancelOrder'))
    if (!ok) return
      ...
      <main className="pt-24 pb-16">
        <div className="max-w-5xl mx-auto px-4">
          ...
          <div className="bg-mimu-white dark:bg-[#1E1E1E] rounded-3xl shadow-sm border border-mimu-cream-border dark:border-[#2A2A2A] p-4 md:p-6 min-h-[300px] mb-8 overflow-hidden">
            ...
            {activeTab === 'orders' && (
              <div>
                <h2 className="text-lg font-bold text-mimu-wine-text dark:text-white mb-4">{t('dashboard.client.myBookings')}</h2>
                {ordersLoading ? (
                  <LocalSpinner />
                ) : orders.length === 0 ? (
                  <div className="animate-fade-in">
                    <p className="text-mimu-wine-light-text dark:text-gray-300/80">{t('dashboard.client.noOrders')}</p>
                    <Link to="/categoria/estadia" className="text-mimu-gold font-medium mt-2 inline-block">{t('dashboard.exploreServices')} →</Link>
                  </div>
                ) : (
            ...
            {activeTab === 'payments' && (
              <div>
                <h2 className="text-lg font-bold text-mimu-wine-text dark:text-white mb-4">{t('dashboard.client.transactionHistory')}</h2>
                {paymentsLoading ? (
                  <LocalSpinner />
                ) : payments.length === 0 ? (
                  <div className="animate-fade-in">
                    <p className="text-mimu-wine-light-text dark:text-gray-300/80">{t('dashboard.client.noPayments')}</p>
                  </div>
                ) : (
                  <div className="space-y-4 animate-fade-in">
                    {payments.map(p => {
                      ...
                      if (p.status === 'paid') { statusBadge = 'bg-green-100 text-green-800'; statusText = t('status.paid') }
                      else if (p.status === 'failed') { statusBadge = 'bg-red-100 text-red-800'; statusText = t('status.failed') }
                      else if (p.status === 'cancelled') { statusBadge = 'bg-gray-100 text-gray-800'; statusText = t('status.cancelled') }
                      else if (p.status === 'refunded') { statusBadge = 'bg-blue-100 text-blue-800'; statusText = t('status.refunded') }
                      return (
                        <div key={p.id} className="p-4 bg-mimu-cream dark:bg-[#121212]/50 border border-transparent rounded-xl flex flex-wrap justify-between items-center gap-4 transition-all duration-300 hover:shadow-sm">
                          <div>
                            ...
                            <div className="text-xs text-mimu-wine-light-text dark:text-gray-400 mt-1 space-y-0.5">
                              <p>{t('dashboard.ref')}: <span className="font-mono">{p.transaction_reference}</span></p>
                              <p>{t('dashboard.method')}: {p.payment_method === 'wallet' ? '💰 ' + t('dashboard.client.wallet') + ' ' : p.payment_method}</p>
                              <p>{t('dashboard.date')}: {new Date(p.created_at).toLocaleString('pt-AO')}</p>
                            </div>
                          </div>
            ...
            {activeTab === 'wallet' && (
              <div>
                <h2 className="text-lg font-bold text-mimu-wine-text dark:text-white mb-4">{t('dashboard.client.myWallet')}</h2>
                {walletLoading ? (
                  <LocalSpinner />
                ) : (
                  <div className="space-y-4 animate-fade-in">
                    <div className="relative overflow-hidden rounded-2xl p-6" style={{ background: 'linear-gradient(135deg, #7B1D1D 0%, #450A0A 100%)' }}>
                      ...
                      <p className="text-white/60 text-xs font-semibold uppercase tracking-wider mb-2">{t('dashboard.availableBalance')}</p>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {[
                        { emoji: '⬇️', label: t('dashboard.deposit'), path: '/carteira' },
                        { emoji: '⬆️', label: t('dashboard.withdraw'), path: '/carteira' },
                        { emoji: '↔️', label: t('dashboard.transfer'), path: '/carteira' },
                      ].map(a => (
      ...
      {reviewOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-mimu-white dark:bg-[#1E1E1E] rounded-3xl border border-mimu-cream-border dark:border-[#2A2A2A] p-6 max-w-md w-full shadow-2xl relative">
            <h3 className="text-lg font-bold text-mimu-wine-text dark:text-white mb-4">{t('dashboard.reviewService')} - {reviewOrder.serviceName}</h3>
            
            {reviewError && <div className="mb-4 p-3 bg-red-100 text-red-700 rounded-xl text-sm">{reviewError}</div>}
            
            <form onSubmit={submitReview}>
              <div className="mb-4">
                <label className="block text-sm font-medium text-mimu-wine-text dark:text-white mb-2">{t('dashboard.ratingLabel')}</label>
              ...
              <div className="mb-6">
                <label className="block text-sm font-medium text-mimu-wine-text dark:text-white mb-2">{t('dashboard.reviewComment')}</label>
                <textarea
              ...
              <div className="flex justify-end gap-3">
                ...
                >
                  {reviewLoading ? t('dashboard.submittingReview') : t('dashboard.submitReview')}
                </button>
              </div>
            </form>
          </div>
            ...
            <div className="overflow-y-auto flex-1 pr-1 space-y-6">
              ...
              <div className="p-5 bg-gradient-to-br from-mimu-cream/80 to-mimu-cream/30 dark:from-[#121212]/50 dark:to-[#121212]/20 border border-mimu-cream-border dark:border-[#2A2A2A] rounded-2xl shadow-sm relative overflow-hidden">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm mt-2">
                  <div>
                    <p className="text-xs text-mimu-wine-light-text/60 dark:text-gray-400 font-medium">{t('dashboard.date')}</p>
                    <p className="font-semibold">{selectedOrder.date}</p>
                  </div>
                  {selectedOrder.time && (
                    <div>
                      <p className="text-xs text-mimu-wine-light-text/60 dark:text-gray-400 font-medium">{t('booking.selectTime').replace('Seleccionar ', '')}</p>
                      <p className="font-semibold">{selectedOrder.time}</p>
                    </div>
                  )}
                  {selectedOrder.guests && (
                    <div>
                      <p className="text-xs text-mimu-wine-light-text/60 dark:text-gray-400 font-medium">{t('booking.guests')}</p>
                      <p className="font-semibold">{selectedOrder.guests}</p>
                    </div>
                  )}
                  <div>
                    <p className="text-xs text-mimu-wine-light-text/60 dark:text-gray-400 font-medium">{t('dashboard.paymentMethod')}</p>
                    <p className="font-semibold">{selectedOrder.paymentMethod || 'A definir'}</p>
                  </div>
                  <div>
                    <p className="text-xs text-mimu-wine-light-text/60 dark:text-gray-400 font-medium">{t('dashboard.statusLabel')}</p>
                    <p className="font-semibold">{statusLabels[selectedOrder.paymentStatus] || selectedOrder.paymentStatus || 'Pendente'}</p>
                  </div>
                  <div>
                    <p className="text-xs text-mimu-wine-light-text/60 dark:text-gray-400 font-medium">{t('listing.price')}</p>
                ...
                {selectedOrder.specialRequests && (
                  <div className="mt-4 pt-3 border-t border-mimu-cream-border dark:border-[#2A2A2A]/60">
                    <p className="text-xs text-mimu-wine-light-text/60 dark:text-gray-400 font-medium mb-1">{t('booking.specialRequests')}</p>
              ...
              <div>
                <h4 className="font-bold text-base mb-3">{t('dashboard.providerInfo')}</h4>
                {loadingProfile ? (
                  <LocalSpinner />
                ) : ownerProfile ? (
                  ...
                  <p className="text-sm text-mimu-wine-light-text/60 dark:text-gray-400 italic">{t('dashboard.providerInfoError')}</p>
                )}
              </div>
            </div>
