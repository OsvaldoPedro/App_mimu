...
import AngolaLocationSelect from '../components/AngolaLocationSelect' // new component
...
export default function RegisterProviderPage() {
  ...
  const [form, setForm] = useState({
    name: '', email: '', phone: '', password: '', province: '', city: '', address: '',
    categoryId: '', serviceTypes: [], hours: ''
  })
  const [documents, setDocuments] = useState({})
  const [documentErrors, setDocumentErrors] = useState({})
  const [error, setError] = useState('')
  const [locationError, setLocationError] = useState('') // track province/city error
  ...
  // single-selection: only one service allowed
  ...
  const handleSubmit = (e) => {
    ...
    if (!form.province || !form.city) {
      setLocationError('Selecciona província e município')
      return
    }
    ...
    <div className="min-h-screen bg-[#F4E8D8]">
      ...
      <main className="pt-24 pb-16 px-4">
        <div className="max-w-2xl mx-auto bg-white rounded-2xl shadow-xl p-8">
          ...
          <h1 className="text-2xl font-bold text-[#3A0D0D] mb-2">{t('provider.registerTitle')}</h1>
          <p className="text-[#5C1A1A]/80 mb-2">A conta ficará "Pendente de Aprovação" até validação.</p>
          <p className="text-amber-700 text-sm mb-6">{t('provider.registerSubtitle')}</p>

          <form onSubmit={handleSubmit} className="space-y-4">
            {error && <div className="p-3 bg-red-100 text-red-700 rounded-xl text-sm">{error}</div>}
            <div>
              <label className="block text-sm font-medium text-[#3A0D0D] mb-2">{t('provider.fullName')}</label>
              <input name="name" value={form.name} onChange={handleChange} required
                className="w-full px-4 py-3 rounded-xl border-2 border-[#F4E8D8] focus:border-[#C58A2B] focus:outline-none" />
            </div>
            <div>
              <label className="block text-sm font-medium text-[#3A0D0D] mb-2">{t('provider.category')}</label>
            ...
            {category && (
              <div>
                <label className="block text-sm font-medium text-[#3A0D0D] mb-2">{t('provider.servicesOffered')}</label>
            ...
            {form.categoryId && (
              <div className="border-2 border-[#F4E8D8] rounded-xl p-4 bg-[#F4E8D8]/30">
                <h3 className="font-bold text-[#3A0D0D] mb-3">{t('provider.requiredDocuments')}</h3>
                {requiredDocs.required.map(docType => (
                  <div key={docType} className="mb-4">
                    ...
                    <label className="inline-flex items-center px-4 py-2 bg-[#C58A2B]/10 border-2 border-dashed border-[#C58A2B] rounded-xl cursor-pointer hover:bg-[#C58A2B]/20 text-[#C58A2B] text-sm font-medium">
                      + Anexar {docType === 'certificates' ? 'ficheiros' : 'ficheiro'}
                ...
                {requiredDocs.optional.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-[#C58A2B]/50">
                    <h4 className="text-sm font-medium text-[#3A0D0D] mb-3">{t('provider.optionalDocuments')}</h4>
            ...
            <div>
              <label className="block text-sm font-medium text-[#3A0D0D] mb-2">{t('provider.hours')}</label>
            ...
            <button type="submit" className="w-full py-4 bg-[#C58A2B] hover:bg-[#E0B15C] text-[#3A0D0D] font-bold rounded-xl transition-colors">
